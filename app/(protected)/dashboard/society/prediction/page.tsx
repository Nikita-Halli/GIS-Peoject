...
const riskBg = (r: string) =>
  r === 'High'   ? 'bg-red-50 border-red-200 text-red-800' :
...
export default function PredictionPage() {
  ...
  return (
    <div className="space-y-6 p-6 max-w-6xl mx-auto">
      ...
      {/* ── RESULT PANEL (full width, above form) ── */}
      {result && (
        <div className="grid md:grid-cols-3 gap-4">

          {/* Risk summary */}
          <div className={`md:col-span-1 border rounded-xl p-6 ${riskBg(result.risk_level)}`}>
            <div className="flex items-center justify-between mb-3">
              <h2 className="font-bold text-base">Prediction Result</h2>
            ...
            <div className="grid grid-cols-2 gap-4 mb-3">
              <div>
                ...
                <p className="text-4xl font-bold">{result.predicted_cases}</p>
                <p className="text-xs opacity-60 mt-1">this month</p>
              </div>
              <div>
                <p className="text-xs opacity-60 mb-1">Confidence</p>
                <p className="text-4xl font-bold">{(result.confidence * 100).toFixed(0)}%</p>
                <p className="text-xs opacity-60 mt-1">certainty</p>
              </div>
            </div>
            <p className="text-xs leading-relaxed opacity-80">{result.interpretation}</p>
          </div>

          {/* Probability bars */}
          <div className="border rounded-xl p-5 bg-white">
            <h3 className="font-bold mb-4 text-sm text-gray-700">Risk Probability</h3>
              ...
              .map(([level, prob]) => (
                <div key={level} className="mb-3">
                  ...
                  <div className="w-full bg-gray-100 h-2.5 rounded-full">
                    <div
                      className={`h-2.5 rounded-full transition-all duration-700 ${riskBar(level)}`}
      ...
      <div className="border rounded-xl p-6 bg-white shadow-sm">
        ...
        <div className="mb-6">
          ...
          <p className="text-xs text-gray-400 mt-1.5">Changing month auto-fills Dharwad climate averages</p>
        </div>

        {/* Climate inputs — 3 columns on wide screens */}
        <div className="mb-6">
          <p className="text-xs font-semibold text-gray-500 uppercase tracking-widest mb-3">Climate Data</p>
        ...
        {/* Environmental sliders — 3 columns */}
        <div className="mb-6">
          <p className="text-xs font-semibold text-gray-500 uppercase tracking-widest mb-3">Environmental Factors</p>
        ...
        <button
          ...
          disabled={loading}
          className="w-full py-3 bg-blue-600 hover:bg-blue-700 active:bg-blue-800 text-white font-semibold rounded-xl transition disabled:opacity-60 flex items-center justify-center gap-2 text-base shadow-sm"
        >
          {loading
      ...
      {/* ── Seasonal Reference Table (full width) ── */}
      <div className="border rounded-xl p-5 bg-white shadow-sm">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b text-gray-500 text-left bg-gray-50">
                <th className="py-3 px-4 font-semibold rounded-tl-lg">Month</th>
                ...
                <th className="py-3 px-4 font-semibold rounded-tr-lg">Risk</th>
              </tr>
            </thead>
            <tbody>
        ...
        <p className="text-xs text-gray-400 mt-3">
          Climate data sourced from KSNDMC, UAS Dharwad MARS station (1991–2024) and Weather Atlas.
          Case counts from Karnataka Vector Borne Disease surveillance (2019–2024).
        </p>
      </div>
...
function NumField({
  icon, label, name, value, onChange, step = 1, min, max,
}: {
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-1">
        ...
        <span className="text-sm font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded">{value}</span>
      </div>
      <input
        type="range"
        name={name}
